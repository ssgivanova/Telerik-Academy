...
class BullsAndCows
 {
        static void Main()
        {
            ...
            //author solution - 4 loops for every digit

            //second solution - loop 1000,9999 -> exclude digis 0
 
            //third solution - 2 arrays
            int cowsN= 0;
            int bullsN = 0;

            char usedDigit = '*';
            char usedSecretDigit = '@';
            ...
            for (int i = 0; i < secret.Length; i++)
            {
                ...
                {
                    if (secret[i] == digits[j])
                    {
                        cowsN++;
                        digits[j] = usedDigit;
                        secret[i] = usedSecretDigit;
                    }
                }
            }
